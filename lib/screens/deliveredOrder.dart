...
import '../widgets/deliveredOrderItem.dart';
import '../models/buyer.dart';

class DeliveredOrder extends StatefulWidget {
  static const routeName = '/DeliveredOrder-screen';
  final Map<String, dynamic> receivedMap;

  const DeliveredOrder({Key? key, required this.receivedMap}) : super(key: key);
...
class _DeliveredOrderState extends State<DeliveredOrder> {
  ...
  void getDeliveredOrders(int buyerId, String buyerName) {
    ServerHandler().getCompletedOrderBuyer(buyerId, buyerName).then((value) => {
      setState(() {
        print("Value of DeliveredOrder Orders $value");
        _buyModel = value;
      })
    });
  ...
  Widget build(BuildContext context) {
    if (_firstExec == true) {
      getDeliveredOrders(int.parse(widget.receivedMap['id']), widget.receivedMap['name']);
      _firstExec = false;
    }
                  ...
                  Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) => DeliveredOrderItem(
