...
class Registration_buyer extends StatefulWidget {
  static const routeName = '/register-screen-seller';

  const Registration_buyer({Key? key}) : super(key: key);

  @override
  State<Registration_buyer> createState() => _Registration_buyerState();
}

class _Registration_buyerState extends State<Registration_buyer> {
  File? _image;
  ...
  Future UploadImage() async {
    final uri = Uri.parse("http://10.0.2.2:/shopice/api/buyer/register");
    ...
    if (response.statusCode == 200) {
      showModalBottomSheet(context: context, builder: (context){
        return Wrap(children: [ListTile(leading: Icon(Icons.done_outline), title: Text('BUYER REGISTERED SUCCESSFULLY'),)],);
      });
    }
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Padding(
              padding: const EdgeInsets.only(top: 20.0, left: 3.0, right: 3.0, bottom: 20.0),
              child: TextField(
